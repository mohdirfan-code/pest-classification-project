import torch, timm, io, os, pandas as pd, requests
...
# --- Configuration ---
# Get the model URL from an environment variable or use the placeholder.
# In Render, we will set the environment variable.
MODEL_URL = os.environ.get("MODEL_URL", "PASTE_YOUR_MODEL_URL_HERE")
MODEL_DEST_PATH = Path("/data/model.pt") # Render provides a writable /data directory
PESTICIDE_DATA_PATH = "data/Pesticides.csv" # Path relative to the project root
NUM_CLASSES = 132
IMG_SIZE = 224
...
async def lifespan(app: FastAPI):
    ...
    # Download the model from the URL if it doesn't exist in our persistent storage
    if not MODEL_DEST_PATH.exists():
        ...
        try:
            ...
            # Allow app to start, but model will be None
            pass
    ...
    try:
        ...
        # Note: For deployment, the script runs from the project root, so the path is direct.
...
async def recommend(request: PestNameRequest):
    ...
    mock_recommendations = {
        "bollworm": { "pest_name": "Bollworm", "pest_info": "The cotton bollworm is a highly destructive pest...", "ipm_solutions": ["Introduce natural predators like Trichogramma wasps.", "Use pheromone traps."], "chemical_solutions": [{"pesticide": ml_models["pesticide_data"].get(pest_name, "N/A"), "dosage": "As per label", "notes": "Follow safety guidelines."}], "prevention_tips": ["Practice crop rotation.", "Ensure timely sowing."] }
    }
    recommendation = mock_recommendations.get(pest_name.lower())
    if not recommendation:
        return { "pest_name": pest_name.replace('_', ' ').title(), "pest_info": "No detailed information available for this pest.", "ipm_solutions": ["Monitor crop regularly.", "Consult local agricultural extension for advice."], "chemical_solutions": [{"pesticide": ml_models["pesticide_data"].get(pest_name, "Consult expert"), "dosage": "N/A", "notes": "N/A"}], "prevention_tips": ["Maintain good field sanitation."] }
